...
export default function TripFaqsPage({
  params,
}: {
  ...
  const handleAddFaq = async (
    ...
    if (faqId && data.images && data.images.length > 0) {
      ...
      for (let i = 0; i < totalImages; i++) {
        ...
        onProgress?.(
          totalProgress,
          `กำลังอัปโหลดรูปภาพ ${i + 1}/${totalImages}...`
        );
  ...
  const handleDeleteFaq = async (faqId: string) => {
    if (!confirm("คุณแน่ใจหรือไม่ที่จะลบ FAQ นี้? รูปภาพทั้งหมดจะถูกลบด้วย"))
      return;
