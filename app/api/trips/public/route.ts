...
export async function GET(request: NextRequest) {
  try {
    ...
    const buildQuery = () => {
      ...
      if (month && month !== "ทุกเดือน") {
        ...
        const endDate = new Date(parseInt(year), parseInt(monthNum), 0)
          ...
          .split("T")[0];
        query = query
          .gte("trip_schedules.departure_date", startDate)
    ...
    if (country && country !== "ทั่วหมด") {
      ...
      countQuery = supabase
        .from("trips")
        .select("id, country:countries!inner(name_th)", {
          count: "exact",
          head: false,
        })
        .eq("is_active", true)
    ...
    const tripMap = new Map<
      string,
      TripRecord & { schedules: ScheduleSummary[] }
    >();
    ...
    tripMap.forEach((trip) => {
      ...
      const slots = formatSlotsDisplay(
        nextSchedule.available_seats || 0,
        nextSchedule.total_seats || 0
      );
      ...
      const schedulesDisplay = sortedSchedules.map((schedule) => {
        ...
        return {
          ...
          dates: formatThaiDateRange(
            schedule.departure_date,
            schedule.return_date
          ),
          duration: formatDurationThai(
            schedDuration.days,
            schedDuration.nights
          ),
      ...
      const title =
        trip.trip_type === "private" ? `[ Private ] ${trip.title}` : trip.title;
