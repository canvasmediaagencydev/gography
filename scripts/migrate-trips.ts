...
async function migrateTrips() {
  ...
  for (const trip of HARDCODED_TRIPS) {
    try {
      ...
      if (!countryId) {
        console.log(`   ⚠️  Warning: Country "${trip.country}" not found, skipping...`)
        errorCount++
        continue
      }
      ...
      if (trip.dates.includes('-') && !trip.dates.includes(' - ')) {
        ...
        if (match) {
          ...
          departureDate = `${year}-${String(month).padStart(2, '0')}-${day1.padStart(2, '0')}`
          returnDate = `${year}-${String(month).padStart(2, '0')}-${day2.padStart(2, '0')}`
        } else {
          console.log(`   ⚠️  Warning: Could not parse dates "${trip.dates}", using defaults`)
          departureDate = `${year}-02-13`
          returnDate = `${year}-02-20`
        }
      } else if (trip.dates.includes(' - ')) {
        // Format: "29 ธ.ค. - 6 ม.ค." (year transition)
        const match = trip.dates.match(/(\d+)\s+([ก-๙\.]+)\s*-\s*(\d+)\s+([ก-๙\.]+)/)
        if (match) {
          ...
          departureDate = `${year1}-${String(month1).padStart(2, '0')}-${day1.padStart(2, '0')}`
          returnDate = `${year2}-${String(month2).padStart(2, '0')}-${day2.padStart(2, '0')}`
        } else {
          departureDate = `${year}-02-13`
          returnDate = `${year}-02-20`
        }
      ...
      if (scheduleError) {
        console.log(`   ⚠️  Trip created but schedule failed:`, scheduleError.message)
      } else {
